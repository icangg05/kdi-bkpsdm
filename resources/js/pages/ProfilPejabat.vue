...
<template>
  ...
  <AppLayout>
    ...
    <div class="container mt-[4rem]">
      ...
      <!-- Card -->
      <!-- Card Highlight untuk data pertama -->
      <div v-if="data.length > 0" class="mt-12">
        <div
          class="bg-white rounded-2xl shadow-lg p-8 text-center flex flex-col items-center max-w-xl mx-auto relative overflow-hidden border-t-4 border-sky-500">
          <!-- Foto profil besar -->
          <div class="size-40 rounded-full overflow-hidden border-4 border-sky-500 shadow mb-6">
            <img :src="data[0].foto ? `/storage/${data[0].foto}` : '/img/default-user.jpg'" alt="Foto Pejabat Utama"
              class="w-full h-full object-cover object-top" />
          </div>

          <!-- Nama -->
          <h3 class="text-xl font-bold text-gray-900">{{ data[0].nama }}</h3>

          <!-- Jabatan -->
          <p v-if="data[0].jabatan?.nama" class="text-base text-gray-700 font-medium">
            {{ data[0].jabatan.nama }}
          </p>

          <!-- Unit Organisasi -->
          <p v-if="data[0].jabatan?.unit_organisasi?.nama" class="text-sm text-gray-500 mt-1">
            {{ data[0].jabatan.unit_organisasi.nama }}
          </p>

          <!-- Bagian -->
          <p v-if="data[0].jabatan?.unit_organisasi?.bagian?.nama && (data[0].jabatan.unit_organisasi.bagian.nama != data[0].jabatan.unit_organisasi.nama)"
            class="text-sm text-gray-500">
            {{ data[0].jabatan.unit_organisasi.bagian.nama }}
          </p>

          <!-- Tombol lihat profil -->
          <Link :href="route('profil.pejabat-detail', data[0].id)"
            class="mt-5 text-white bg-sky-600 px-5 py-2 rounded-full hover:bg-sky-700 transition">
          Lihat Profil
          </Link>
        </div>
      </div>

      <!-- Grid untuk data sisanya -->
      <div v-if="data.length > 1" class="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div v-for="(item, index) in data.slice(1)" :key="index"
          ...
          <!-- Bagian -->
          <p v-if="item.jabatan?.unit_organisasi?.bagian?.nama && (item.jabatan.unit_organisasi.bagian.nama != item.jabatan.unit_organisasi.nama)"
