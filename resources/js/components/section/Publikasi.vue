        ...
        <div :key="activeCategory" class="grid md:grid-cols-2 gap-6 items-start">
          ...
          <div class="rounded-xl overflow-hidden shadow-lg" v-if="publikasiUtama">
            ...
            <div class="bg-sky-600 text-white px-6 py-4">
              ...
              <p class="mt-1 flex items-center text-sm opacity-80">
                <Calendar class="w-4 h-4 mr-1 text-gray-100" />
                {{ publikasiUtama.tanggal }}
              </p>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-4" v-if="publikasiLain.length > 0">
            <div v-for="item in publikasiLain" :key="item.id"
              ...
              <!-- Sampul (Thumbnail) -->
              <img :src="item.sampul ? `/storage/${item.sampul}` : '/img/default-publikasi.png'" alt="Sampul"
                class="aspect-[3/2.2] w-28 object-cover rounded-md" />

              <!-- Konten -->
              <div>
                <Link :href="route('berita.show', item.slug)"
                  class="hover:underline font-medium text-gray-800 leading-tight text-sm lg:text-base line-clamp-2">
                {{ item.judul }}
                </Link>
                <p class="mt-1 flex items-center text-sm text-gray-500">
                  <Calendar class="w-4 h-4 mr-1 text-gray-500" />
                  {{ item.tanggal }}
                </p>
              </div>
            </div>
